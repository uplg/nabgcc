...
static void eapol_input_msg3(uint8_t *frame, uint32_t length)
{
	...
	if(memcmp(fr_in->key_frame.key_nonce, anonce, EAPOL_NONCE_LENGTH) != 0)
  {
    ...
    DIAG_EAPOL_EV(DIAG_EAPOL_MICFAIL);
    return;
  }
   ...
   * whole life of the association. That was this driver until now: the
   * counters showed eg=0 and SEC_CSR0=1 (pairwise slot only) forever,
   * and the "wedge" was simply peers ageing us out of their ARP caches
   * and re-ARPing by broadcast into the void.
   */
  if((ieee80211_encryption&0x0F) == IEEE80211_CIPHER_CCMP)
 ...
 * slot untouched, returns 0 and says so on the diag channel: a wrong key
 * installed silently is what cost us three nights.
 */
static int32_t eapol_install_msg3_gtk(struct eapol_frame *fr_in,
                                      uint32_t length)
{
   ...
   * frame the radio actually delivered. */
  if((wlen < 24) || (wlen % 8) || (wlen > sizeof(plain)+8) ||
     (sizeof(struct eapol_frame) + wlen > length)) {
    DBG_WIFI("GTK: implausible key data length"EOL);
    DIAG_EAPOL_EV(DIAG_EAPOL_GTKFAIL);
    return 0;
  }
  ...
  rt2501_set_key(key_id, gtk.s.ek, gtk.s.mick.tx, gtk.s.mick.rx,
                 RT2501_CIPHER_AES);
  DIAG_EAPOL_EV(DIAG_EAPOL_GTKOK);
  return 1;
}
   ...
   */
			...
			(fr->key_frame.key_info.request == 0))
	{
		...
		eapol_input_msg3(frame, length);
			...
			(fr->key_frame.key_info.request == 0))
	{
		DIAG_EAPOL_EV(DIAG_EAPOL_GROUP);
