...
/*
 * Real AES-128, decrypt direction only (what RFC 3394 key unwrap needs;
 * the CCMP data path is done by the RT2573 hardware). Byte-oriented, two
 * 256-byte tables, no encrypt side: flash is the scarce resource here.
 * Wired into eapol.c (E2 of docs/nabaztag-eapol-campaign.md): message 3/4
 * and group rekeys unwrap the GTK through aes_key_unwrap + rsn_find_gtk.
 * The no-op legacy API above has no callers left and awaits deletion once
 * the fix is proven on hardware.
 */
